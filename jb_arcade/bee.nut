...
KeyboardLineFour<- [-1,  " ", " ", " ", " ", " ", " ", "",  "",  "" ]; //clear and space bar
...
NumpadLineOne<-  ["" , "" , "" , "7", "8", "9", "" , "" , "" , "" ];
NumpadLineTwo<-  ["" , "" , "" , "4", "5", "6", "" , "" , "" , "" ];
NumpadLineThree<-["" , "" , "" , "1", "2", "3", "" , "" , 0  , 0  ]; //delete
NumpadLineFour<- [-1 , "" , "" , "" , "0", "" , "" , "" , "" , "" ]; //clear
NumpadLines<-[NumpadLineOne, NumpadLineTwo, NumpadLineThree, NumpadLineFour];
...
function Setup()
{
	...
	EasyButtonTT = Entities.FindByName(null, "bee_randomword_easy_tt");
	MediumButtonTT = Entities.FindByName(null, "bee_randomword_medium_tt");
	HardButtonTT = Entities.FindByName(null, "bee_randomword_hard_tt");
	KeyboardTT = Entities.FindByName(null, "bee_tt_keyboard");
	CurrentGameButtonTT[0] = Entities.FindByName(null, "bee_spell_tt");
	CurrentGameButtonTT[1] = Entities.FindByName(null, "bee_math_tt");
	
	Damage = 0;
	EntFireByHandle(DamageSignTT, "SetTextureIndex", DAMAGE_TO_TEXTURE[Damage].tostring(), 0.0, null, self);
	EntFireByHandle(CurrentGameButtonTT[CurrentGame], "SetTextureIndex", "1", 0.0, null, self);
}
...
function SetGame(game) {
	...
	EntFireByHandle(CurrentGameButtonTT[CurrentGame], "SetTextureIndex", "0", 0.0, null, self);
	EntFireByHandle(CurrentGameButtonTT[game], "SetTextureIndex", "1", 0.0, null, self);
	...
	EntFireByHandle(KeyboardTT, "SetTextureIndex", game.tostring(), 0.0, null, self);
	...
	EntFireByHandle(EasyButtonTT, "SetTextureIndex", "0", 0.0, null, self);
	EntFireByHandle(MediumButtonTT, "SetTextureIndex", "0", 0.0, null, self);
	EntFireByHandle(HardButtonTT, "SetTextureIndex", "0", 0.0, null, self);
}

function ChooseNewWordOrProblem(difficulty) {
	if (CurrentGame == GAME_SPELLING) {
		ChooseNewWord(difficulty);
	} else {
...
function PressButton(booth)
{
	...
	local key = (CurrentGame == GAME_SPELLING) ? KeyboardLines[row][column] : NumpadLines[row][column];
	...
	if (key == 0)
	{
		...
		if (BoothInput[booth].len() > 0)
		{
			if (CurrentGame == GAME_SPELLING) {
				...
				BoothInput[booth] <- BoothInput[booth].slice(1);
			}
		}
	}
	else if (key == -1)
	...
	else if (key == " " && BoothInput[booth] == "")
	{
		//Special case: don't allow space at the start of the word
		debugprint("Ignoring space at start of word");
	}
	else if (BoothInput[booth].len() < ((CurrentGame == GAME_SPELLING) ? MAX_WORD_SIZE : MAX_NUMBER_SIZE))
	{
		//Add the key to the booth's input
		if (CurrentGame == GAME_SPELLING) {
			BoothInput[booth] += key;
		} else {
			BoothInput[booth] = key + BoothInput[booth];
		}
	}
...
function UpdateInputEntity(booth, color)
{
	...
	local input = "";
	local cursor = (TimeLeft > 0 && BoothInput[booth].len() < MAX_NUMBER_SIZE) ? "_" : "";
	if (GameStarted) {
		input = (CurrentGame == GAME_SPELLING) ? BoothInput[booth] : format("%s%4s",CurrentProblem, cursor + BoothInput[booth]);
	}
	local inputAndTimer = RightAlignText(input, TimeLeft.tostring(), MAX_WORD_SIZE + 3); 
...
function CheckSpelling(booth)
{
	...
	if (booth in BoothInput)
	{
		...
		local correctAnswer = CurrentGame == GAME_SPELLING ? SpellWord : CurrentAnswer;
		if (correctAnswer == BoothInput[booth])
		{
...
function CheckSpellingAll()
{
	...
	local answerText = CurrentGame == GAME_SPELLING ? ("Correct spelling of word: \x05" + SpellWord + "\x01 ") : ("Correct solution: \x05" + CurrentProblem + CurrentAnswer + "\x01 ")
	ScriptPrintMessageChatAll(answerText);
	if ((CurrentGame == GAME_SPELLING && SpellWord.len() > 0) || (CurrentGame == GAME_MATH && CurrentAnswer.len() > 0)) {
		for (local booth = 1; booth <= NUM_BOOTHS; booth++)
		{
			CheckSpelling(booth);
		}
...
function StartTimer()
{
	if (TimerActive || (SpellWord == "" && CurrentProblem == "")) return;
	GameNumber++;
	GameStarted = true;
	TimerActive = true;
...
function AddToTime(toAdd)
{
	if (toAdd < 0 && TimeLeft + toAdd < MIN_TIME) return;
...
function DisplayHelpText()
{
	...
	while ((ply = Entities.FindByClassname(ply, "player")) != null)
	{
		...
		{
			if (CurrentGame == GAME_SPELLING) {
				DisplayText(ply, "Instructions\n1. Listen to the word and definition.\n2. When the bell rings you can begin.\n3. Use the keyboard in your booth to enter the word.");
			} else {
				DisplayText(ply, "Instructions\n1. Once the bell rings, solve the math problem.\n2. Use the keypad in your booth to enter the answer.");
			}
		}
			...
			playerOrigin.z >= CONTROLS_AREA_MIN.z && playerOrigin.z < CONTROLS_AREA_MAX.z)
		{
			if (CurrentGame == GAME_SPELLING) {
				...
				DisplayText(ply, "Instructions\n1. Select damage for wrong answers\n2. Choose difficulty to get a math problem\n3. Start the timer and the players will see the problem");
			}
		}
	}
}
...
function DisplayProblem(booth)
{
	...
	if (game_txt != null)
	{
		game_txt.__KeyValueFromString("message", CurrentProblem + BoothInput[booth])
		EntFireByHandle(game_txt,"Display","",0.05,player,player);
		return;
	}
	debugprint("no game_txt found");
...
{
	local problemType = RandomInt(0, 2);
	switch (problemType)
	{
		case 0:
			local firstNumber = RandomInt(10, 99);
			local secondNumber = RandomInt(10, 99);
			CurrentProblem = firstNumber.tostring() + "+" + secondNumber.tostring();
			CurrentAnswer = (firstNumber + secondNumber).tostring();
			break;
		case 1:
			local firstNumber = RandomInt(3, 5);
			local secondNumber = RandomInt(20, 99);
			CurrentProblem = firstNumber.tostring() + "*" + secondNumber.tostring();
			CurrentAnswer = (firstNumber * secondNumber).tostring();
			break;
		case 2:
			local firstNumber = RandomInt(3, 9);
			local secondNumber = RandomInt(4, 9);
			local dividend = firstNumber * secondNumber;
			CurrentProblem = dividend.tostring() + "/" + secondNumber.tostring();
			CurrentAnswer = (firstNumber).tostring();
			break;
		default:
			debugprint("ERROR: unknown problem type " + problemType + " in GenerateEasyProblem");
			return;
	}
}

function GenerateMediumProblem(includeEasy)
{
	local problemType = RandomInt(includeEasy ? 0 : 1, 2);
	switch (problemType)
	{
		case 0:
			GenerateEasyProblem();
			local thirdNumber = RandomInt(10, 99);
			CurrentProblem = "(" + CurrentProblem + ")+" + thirdNumber.tostring();
			CurrentAnswer = (CurrentAnswer.tointeger() + thirdNumber).tostring();
			break;
		case 1:
			local firstNumber = RandomInt(100, 999);
			local secondNumber = RandomInt(100, 999);
			CurrentProblem = firstNumber.tostring() + "+" + secondNumber.tostring();
			CurrentAnswer = (firstNumber + secondNumber).tostring();
			break;
		case 2:
			local firstNumber = RandomInt(21, 85);
			local secondNumber = RandomInt(21, 85);
			if (firstNumber < secondNumber) {
				local tmp = firstNumber;
			...
			if ((firstNumber % 10) - (secondNumber % 10) >= 0) {
				secondNumber -= (secondNumber % 10) + 1; //last digit is now 9
				firstNumber += (10 - (firstNumber % 10)) //last digit is now 0
				
				secondNumber -= RandomInt(0, 4); //last digit is now between 9 and 5
				firstNumber += RandomInt(0, 4); //last digit is now between 0 and 4;
			}
			CurrentProblem = firstNumber.tostring() + "-" + secondNumber.tostring();
			CurrentAnswer = (firstNumber - secondNumber).tostring();
			break;
		default:
			debugprint("ERROR: unknown problem type " + problemType + " in GenerateMediumProblem");
			return;
	}
}
...
function GenerateHardProblem()
{
	...
	{
		case 0:
			GenerateMediumProblem(false);
			local thirdNumber = RandomInt(10, 99);
			CurrentProblem = "" + CurrentProblem + "+" + thirdNumber.tostring();
			CurrentAnswer = (CurrentAnswer.tointeger() + thirdNumber).tostring();
			break;
		case 1:
			local firstNumber = RandomInt(2, 9);
			local secondNumber = RandomInt(100, 999);
			CurrentProblem = firstNumber.tostring() + "*" + secondNumber.tostring();
			CurrentAnswer = (firstNumber * secondNumber).tostring();
			break;
		case 2:
			local firstNumber = RandomInt(121, 985);
			local secondNumber = RandomInt(21, 89);
			...
			break;
		default:
			debugprint("ERROR: unknown problem type " + problemType + " in GenerateHardProblem");
			return;
	}

}

function ChooseNewProblem(difficulty)
{
	TimerActive = false;
	GameStarted = false;
	switch (difficulty)
	{
		case 0:
			...
			break;
		case 1:
			GenerateMediumProblem(true);
			break;
		case 2:
			GenerateHardProblem();
			break;
		default:
			debugprint("ERROR: unknown difficulty " + difficulty + " in GenerateEasyProblem");
			return;
	}
	
	CurrentProblem += "=";
	DisplayCurrentTargetWord(CurrentProblem);
	TimeLeft = 15 + difficulty * 5;
	SetTimerSeconds(TimeLeft);
	ClearInputs();
	UpdateAllGameTexts();
